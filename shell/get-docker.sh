	...
	case "$lsb_dist" in
		ubuntu|debian|raspbian)
			...
			(
				...
				$sh_c "curl -fsSL \"$DOWNLOAD_URL/linux/$lsb_dist/gpg\" | apt-key add  - "
			...
			(
				...
				if [ -n "$cli_pkg_version" ]; then
					$sh_c "apt-get install -y  --no-install-recommends docker-ce-cli=$cli_pkg_version "
				fi
				$sh_c "apt-get install -y  --no-install-recommends docker-ce$pkg_version "
			)
			echo_docker_as_nonroot
			exit 0
			;;
